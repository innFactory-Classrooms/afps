...
object MainApplication {
  ...
  def asyncApplication(using ec: ExecutionContext): Future[Any] = {
    ...
    val request = HttpRequest
      .newBuilder()
      .uri(URI.create("http://localhost:9000/v1/weather"))
    ...
    val requests = ParVector.range(1, 10)
      .map(requestId => {
      println(s"sending request $requestId")
      Await.result(client.sendAsync(request, HttpResponse.BodyHandlers.ofString()).asScala.map(v =>  
        println(s"requestId ${requestId} response ${v.body()}")
      ), maxTimeout)
    })
